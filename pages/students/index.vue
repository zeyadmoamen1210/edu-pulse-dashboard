              ...
              <div class="col-md-3">
                  <div class="filters">
                      ...
                      <div>
                        ...
                        <div v-if="sectionVal">
                            <vs-button @click="addNewStudentPopup = true"> New Student </vs-button>
                        </div>
                  </div>
              </div>
...
export default {
    ...
    methods:{
        ...
        changeStudentSubject(){
            ...
            this.$axios.patch(`/students/${this.currStudent.id}/path`, {section: this.changeSubject.sectionVal}).then(res => {
                this.$notify({
                    title: 'تم بنجاح!',
                    message: `لقد تم تعديل مادة الطالب بنجاح`,
                    type: 'success'
                });
                this.getStudents();
    ...
    addStudentInBackend(){
        ...
        this.$axios.post(`/students`, {
            "username": this.addStudent.username,
            "email": this.addStudent.email,
            "phone": this.phoneObj.formattedNumber,
            "section":this.sectionVal,
            "password": this.addStudent.password,
            "address":this.addStudent.address,
            "mustChangePassword": this.mustChange ? true : false
        }).then(res => {
            this.addNewStudentPopup = false;
            this.$notify({
                title: 'تم بنجاح!',
                message: `لقد تم إضافة الطالب بنجاح`,
                type: 'success'
            });
            this.getStudents();
        }).finally(() => loading.close());
