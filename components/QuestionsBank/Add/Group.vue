  ...
  <div class="add-true add-new-question">
      ...
      <div class="inner-head">
          <div class="d-flex justify-content-between">
              <div>
                  <h6 class="mt-2"> نوع السؤال: <span>جروب</span></h6>
              </div>
      ...
      <el-form :model="question" ref="questiongroup" :class="{'add-question-form': true, 'arabic-form': $i18n.locale == 'ar', 'english-form': $i18n.locale == 'en'}">
...
export default {
    ...
    methods:{
        ...
        img.onload = function() {
            ...
            if(this.width > 600 && this.height > 200 && this.height < 650 && this.width < 800){
        ...
        saveGroupQuestion(e){
            ...
            if(!(this.question.head && this.question.head.toString().trim() != '')){
                this.$message.error({
                    message:this.$i18n.locale == 'ar' ? "ادخل رأس السؤال او القطعة" : "Head Of Group Question Is Required " ,
                })
                return;
            }
            ...
            let addGroup = this.$axios.post(`/lessons/${this.lesson}/questions`, groupQues).then(res => {
                question = res.data;
            });

            Promise.all([addGroup]).then(res => {
                this.$axios.post(`/questions-group/${question.id}/children`, [...e]).then(res => {

                   this.$message({
                        message:this.$i18n.locale == 'ar' ? "تم إضافة السؤال بنجاح" : "Questions Added Successfully" ,
                        type: "success"
                    });
                    this.question.head = "";
